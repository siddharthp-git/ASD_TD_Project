"""
data_loader.py – DV-STTGAT  (ADOS 0-10, All sites, CC400 atlas)
====================================================
...
Only subjects with ADOS_TOTAL score between 0 and 10 from all sites are retained.
...
def load_from_cache(
    ...
    # ── ADOS 0-10 filter (all sites) ──────────────────────────────────────────
    if "ADOS_TOTAL" in pheno.columns:
        filtered_pheno = pheno[(pheno["ADOS_TOTAL"] >= 0) & (pheno["ADOS_TOTAL"] <= 10)]
    else:
        print("[DataLoader] WARNING: 'ADOS_TOTAL' column not found in phenotype! Using all subjects.")
        filtered_pheno = pheno

    n_total   = len(pheno)
    n_filtered    = len(filtered_pheno)
    print(f"[DataLoader] ADOS 0-10 filter: keeping {n_filtered}/{n_total} subjects.")
    ...
    num_sites   = 1
    print("[DataLoader] All sites (ADOS 0-10). DANN adversarial loss is disabled.")
    ...
    if not records:
        raise RuntimeError(
            "[DataLoader] No ADOS 0-10 subjects matched between CC400 cache and phenotype CSV/Excel!\n"
    ...
    print(
        f"[DataLoader] Matched {len(subject_ids)} ADOS 0-10 subjects "
