"""
tune.py – Optuna Hyperparameter Tuning for DV-STTGAT (ADOS 0-10, All sites, CC400)
=======================================================================
...
def _set_global_seeds(seed: int = _GLOBAL_SEED) -> None:
    """Fix all random sources so every full run produces identical results."""
    random.seed(seed)
    np.random.seed(seed)
...
# ─────────────────────────────────────────────────────────────────────────────
def _compute_metrics(targets: np.ndarray, probs: np.ndarray,
                     threshold: float = 0.5) -> dict:
    """
    Compute a full suite of binary classification metrics.

    Returns a dict with keys: acc, auc, prec, rec, sens, spec, f1
      • sens (sensitivity) = TP / (TP + FN)  — same as recall for ASD class
      • spec (specificity) = TN / (TN + FP)  — recall for TD  class
    """
    preds = (probs >= threshold).astype(int)
    tgt   = targets.astype(int)

    acc  = accuracy_score(tgt, preds)
    prec = precision_score(tgt, preds, zero_division=0)
    rec  = recall_score(tgt, preds, zero_division=0)        # = sensitivity
    f1   = f1_score(tgt, preds, zero_division=0)

    try:
        auc = float(roc_auc_score(tgt, probs))
    except ValueError:
        auc = float("nan")

    cm           = confusion_matrix(tgt, preds, labels=[0, 1])
    tn, fp, fn, tp = cm.ravel()
    sens = tp / (tp + fn) if (tp + fn) > 0 else 0.0
    spec = tn / (tn + fp) if (tn + fp) > 0 else 0.0

    return dict(acc=acc, auc=auc, prec=prec, rec=rec, sens=sens, spec=spec, f1=f1)
...
def _run_tune_fold(
    ...
    *,
    ...
    Reports intermediate val-Accuracy to Optuna after every epoch so the
    MedianPruner can kill bad trials early.

    Returns
    -------
    dict with keys  train_{acc,auc,prec,rec,sens,spec,f1}
                     val_{acc,auc,prec,rec,sens,spec,f1}
    """
    ...
    best_acc         = -1.0          # initialise < 0 so first epoch always saves
    best_val_metrics = {}
    es_counter       = 0
    ...
    for epoch in range(1, epochs + 1):
        ...
        # ── Validation ────────────────────────────────────────────────────────
        # Fixed 0.5 threshold during tuning — sufficient to rank configs;
        # avoids the expensive Youden's-J forward pass used in train.py.
        model.eval()
        ...
        val_metrics = _compute_metrics(np.array(all_targets), np.array(all_probs))
        ...
        # ── Per-fold early stopping + snapshot best val metrics ───────────────
        if val_acc > best_acc:
            best_acc         = val_acc
    ...
    # ── Final train-set eval (one pass, after epoch loop) ─────────────────────
    # Gives training metrics at the model state when training ended.
    model.eval()
    tr_targets, tr_probs = [], []
    with torch.no_grad():
        for batch in train_loader:
            ...
            targets = batch.y.view(-1, 1).float()
            probs   = torch.sigmoid(model(batch))
            tr_targets.extend(targets.cpu().numpy().flatten())
            tr_probs.extend(probs.cpu().numpy().flatten())

    train_metrics = _compute_metrics(np.array(tr_targets), np.array(tr_probs))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Optuna hyperparameter tuning for DV-STTGAT (ADOS 0-10, All sites, CC400)"
    )
    ...
    parser.add_argument("--n_trials",    type=int, default=100,
                        help="Max Optuna trials (default 100)")
    ...
    print("=" * 70)
    print("  DV-STTGAT — Optuna Hyperparameter Tuning  (ADOS 0-10 | All sites | CC400)")
    ...
    print(
        ...
        f"  ⏳ This runs ONCE and is reused for all {args.n_trials} trials."
    )
    ...
    # ─────────────────────────────────────────────────────────────────────────
    def objective(trial):
        # ── Per-trial seed reset (ensures model init + mixup are reproducible) ─
        _set_global_seeds(_GLOBAL_SEED + trial.number)
        ...
        print(f"  [T{trial.number:>4}] lr={lr:.2e}  wd={weight_decay:.2e}"
              f"  fg={focal_gamma:.2f}  fa={focal_alpha:.2f}"
              f"  ma={mixup_alpha:.2f}  mp={mixup_prob:.2f}"
              f"  T0={cosine_t0:>2}  bs={batch_size}")
        print(f"  {'':7}  {'':5}  {'Acc':>6}  {'AUC':>6}  {'Prec':>6}"
              f"  {'Rec':>6}  {'Sens':>6}  {'Spec':>6}  {'F1':>6}")
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(fold_splits):
            ...
            # ── Per-fold metric table row ─────────────────────────────────────
            print(
                f"  {'':7}  Fold{fold_idx+1}  "
                f"Train  "
                f"{m['train_acc']:6.4f}  {m['train_auc']:6.4f}  "
                f"{m['train_prec']:6.4f}  {m['train_rec']:6.4f}  "
                f"{m['train_sens']:6.4f}  {m['train_spec']:6.4f}  "
                f"{m['train_f1']:6.4f}"
            )
            print(
                f"  {'':7}  {'':5}  "
                f"Val    "
                f"{m['val_acc']:6.4f}  {m['val_auc']:6.4f}  "
                f"{m['val_prec']:6.4f}  {m['val_rec']:6.4f}  "
                f"{m['val_sens']:6.4f}  {m['val_spec']:6.4f}  "
                f"{m['val_f1']:6.4f}"
            )
        ...
        mean_val_acc  = _mean("val_acc")
        mean_val_auc  = _mean("val_auc")
        mean_val_prec = _mean("val_prec")
        mean_val_rec  = _mean("val_rec")
        mean_val_sens = _mean("val_sens")
        mean_val_spec = _mean("val_spec")
        mean_val_f1   = _mean("val_f1")

        mean_tr_acc   = _mean("train_acc")
        mean_tr_auc   = _mean("train_auc")
        mean_tr_prec  = _mean("train_prec")
        mean_tr_rec   = _mean("train_rec")
        mean_tr_sens  = _mean("train_sens")
        mean_tr_spec  = _mean("train_spec")
        mean_tr_f1    = _mean("train_f1")

        # ── Per-trial mean summary ────────────────────────────────────────────
        sep = "  " + "-" * 64
        print(sep)
        print(
            f"  {'':7}  Mean   Train  "
            f"{mean_tr_acc:6.4f}  {mean_tr_auc:6.4f}  "
            f"{mean_tr_prec:6.4f}  {mean_tr_rec:6.4f}  "
            f"{mean_tr_sens:6.4f}  {mean_tr_spec:6.4f}  "
            f"{mean_tr_f1:6.4f}"
        )
        print(
            f"  {'':7}  {'':5}  Val    "
            f"{mean_val_acc:6.4f}  {mean_val_auc:6.4f}  "
            f"{mean_val_prec:6.4f}  {mean_val_rec:6.4f}  "
            f"{mean_val_sens:6.4f}  {mean_val_spec:6.4f}  "
            f"{mean_val_f1:6.4f}"
        )
        print(sep)

        # ── Persist all metrics to Optuna DB (queryable after tuning) ─────────
        trial.set_user_attr("val_acc",   mean_val_acc)
        trial.set_user_attr("val_auc",   mean_val_auc)
        trial.set_user_attr("val_prec",  mean_val_prec)
        trial.set_user_attr("val_rec",   mean_val_rec)
        trial.set_user_attr("val_sens",  mean_val_sens)
        trial.set_user_attr("val_spec",  mean_val_spec)
        trial.set_user_attr("val_f1",    mean_val_f1)
        trial.set_user_attr("train_acc",  mean_tr_acc)
        trial.set_user_attr("train_auc",  mean_tr_auc)
        trial.set_user_attr("train_prec", mean_tr_prec)
        trial.set_user_attr("train_rec",  mean_tr_rec)
        trial.set_user_attr("train_sens", mean_tr_sens)
        trial.set_user_attr("train_spec", mean_tr_spec)
        trial.set_user_attr("train_f1",   mean_tr_f1)

        return mean_val_acc   # Optuna maximises mean val Accuracy
