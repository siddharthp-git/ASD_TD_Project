...
6. [Pipeline Step 3: Run Soft-Voting Ensemble Evaluation](#6-pipeline-step-3-run-voting-ensemble-evaluation)
...
```powershell
cd "e:\ASD_TD_Project\dv-sttgat-11-18-cc400-hyper"
```
...
* `AGE_AT_SCAN`: Age of the subject at scan. The data loader filters specifically for subjects with age $\ge 11$ and $\le 18$.
...
1. **[build_cc400_cache.py](file:///e:/ASD_TD_Project/dv-sttgat-11-18-cc400-hyper/build_cc400_cache.py)**
   * Locate **Line 67**:
     ```python
     DEFAULT_DATA_DIR  = r"D:\ABIDE"  # <-- Change this to your raw fMRI data directory
     ```
2. **[train.py](file:///e:/ASD_TD_Project/dv-sttgat-11-18-cc400-hyper/train.py)**
   * Locate **Line 61**:
     ```python
     DEFAULT_PHENOTYPE = r"D:\ABIDE\asd_717participants.csv"  # <-- Change to your phenotype CSV file path
     ```
3. **[ensemble_eval.py](file:///e:/ASD_TD_Project/dv-sttgat-11-18-cc400-hyper/ensemble_eval.py)**
   * Locate **Line 36**:
     ```python
     DEFAULT_PHENOTYPE = r"D:\ABIDE\asd_717participants.csv"  # <-- Change to your phenotype CSV file path
     ```
4. **[tune.py](file:///e:/ASD_TD_Project/dv-sttgat-11-18-cc400-hyper/tune.py)**
   * Locate **Line 141**:
     ```python
     DEFAULT_PHENOTYPE = r"D:\ABIDE\asd_717participants.csv"  # <-- Change to your phenotype CSV file path
     ```

### 4. Changing the Age Range Filter

By default, the data loader filters specifically for subjects with ages in the range [11, 18]. If you wish to change the target age group, open **[data_loader.py](file:///e:/ASD_TD_Project/dv-sttgat-11-18-cc400-hyper/data_loader.py)**:
* Locate **Line 115**:
  ```python
  filtered_pheno = pheno[(pheno["AGE_AT_SCAN"] >= 11) & (pheno["AGE_AT_SCAN"] <= 18)]
  ```
  Modify the `>= 11` and `<= 18` logic to target your custom age limits.
...
1. Loads cached BOLD signals for subjects aged 11 to 18 across all sites.
...
Below is an overview of what each script does in the `dv-sttgat-11-18-cc400-hyper/` directory:

```
dv-sttgat-11-18-cc400-hyper/
├── build_cc400_cache.py            # Extracts BOLD signals from raw fMRI using CC400 atlas.
├── data_loader.py                  # Loads BOLD matrices, filters for Age 11-18, aligns with labels.
