"""
data_loader.py – DV-STTGAT  (NYU-only, CC400 atlas)
====================================================
...
Only subjects from SITE_ID == 'NYU' (or 'ABIDEII-NYU_1') are retained.
...
def load_from_cache(
    ...
    # ── NYU-only filter ───────────────────────────────────────────────────────
    nyu_pheno = pheno[(pheno["SITE_ID"] == "NYU") | (pheno["SITE_ID"] == "ABIDEII-NYU_1")]
    n_total   = len(pheno)
    n_nyu     = len(nyu_pheno)
    print(f"[DataLoader] NYU-only filter: keeping {n_nyu}/{n_total} subjects.")
    pheno = nyu_pheno
    ...
    num_sites   = 1
    print("[DataLoader] Single site (NYU). DANN adversarial loss will be disabled.")
    ...
    if not records:
        raise RuntimeError(
            "[DataLoader] No NYU subjects matched between CC400 cache and phenotype CSV!\n"
    ...
    print(
        f"[DataLoader] Matched {len(subject_ids)} NYU subjects "
